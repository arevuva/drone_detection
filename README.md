...
## ESP32-CAM streaming + детекция дронов

Используйте вес `best.pt` из корня репозитория для детекции дронов в реальном времени с потока ESP32-CAM.

1. **Подготовка ESP32-CAM**
   - Откройте и прошейте скетч `esp32_cam_stream.ino` (AI-Thinker ESP32-CAM). Укажите SSID/PASS своей сети Wi‑Fi.
   - После загрузки посмотрите в Serial Monitor адрес устройства. Поток будет доступен по `http://<IP-адрес>:81/stream`.
...
3. **Запуск детекции**
   ```bash
   python esp32_stream_detect.py --stream-url http://<IP-адрес>:81/stream --weights best.pt --show
   ```
   - В консоли выводится вердикт: обнаружен ли дрон и вероятность (`probability XX.XX%`).
   - Ключ `--show` откроет окно OpenCV с отрисованными боксами (ESC для выхода). Без него скрипт работает только в консоли.
   - Параметр `--conf` регулирует порог уверенности.

Файл `esp32_stream_detect.py` отвечает за прием MJPEG-потока, покадровую подачу кадров в YOLOv8 и вывод вероятности в консоль. Скетч `esp32_cam_stream.ino` поднимает MJPEG‑поток `/stream` на ESP32‑CAM.
